...
public class WiFiDirectBroadcastReceiver extends BroadcastReceiver
{
    ...
    public WiFiDirectBroadcastReceiver(WifiP2pManager manager, Channel channel)
    {
        ...
        if (Util.context instanceof WifiP2pActivity)
        {
            activity = (WifiP2pActivity) Util.context;
        } else
        {
            Util.alert("Error!", "To use WifiDirectBroadcastReciever, Util.context must be an instance of WifiP2pActivity. Please report this error.",
                    new DialogInterface.OnClickListener()
                    {
                        @Override
                        public void onClick(DialogInterface dialogInterface, int i)
                        {
                            if (Util.context instanceof Activity)
                            {
    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action))
        {
            ...
            Log.d("wifidirect", "P2P state changed - " + state);
...
//                DeviceDetailFragment fragment = (DeviceDetailFragment) ((Activity) activity).getFragmentManager().findFragmentById(R.id.frag_detail);
