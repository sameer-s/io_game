...
public class IOGameGLSurfaceView extends GLSurfaceView
{
	...
	public class Renderer implements GLSurfaceView.Renderer
	{
		...
		public void generateObject(int number) // FIXME hardcoded to a 30 x 30 board
		{
			Random r = new Random();
			float x, y;
			...
			if (r.nextBoolean())
				y = y * -1.0f;
			int type = TypeGen();
			if(type==1)toDraw.put(new Square(Util.loadBitmap("drawable/zorua")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + number));
			else if(type==2) toDraw.put(new Square(Util.loadBitmap("drawable/victini")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + number));
			else if(type==3) toDraw.put(new Square(Util.loadBitmap("drawable/psyduck")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + number));
			else if(type==4) toDraw.put(new Square(Util.loadBitmap("drawable/fennekin")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + number));
		...
		public void intialObject(){
			for(int i = 0; i<16; i++){
				...
				if (r.nextBoolean())
					y = y * -1.0f;
				int type = TypeGen();
				if(type==1)toDraw.put(new Square(Util.loadBitmap("drawable/zorua")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + i));
				else if(type==2) toDraw.put(new Square(Util.loadBitmap("drawable/victini")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + i));
				else if(type==3) toDraw.put(new Square(Util.loadBitmap("drawable/psyduck")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + i));
				else if(type==4) toDraw.put(new Square(Util.loadBitmap("drawable/fennekin")).setState(0f, x, y, 0.4f, 0.5f).setName("object" + i));
		...
		public void onDrawFrame(GL10 unused)
		{
			...
			for( int i = 0; i<16; i++){
				if (checkCollision(toDraw.get("player" + rand), toDraw.get("object" + i))){
					toDraw.remove("object" + i);
					generateObject(i);
				}
			}
		}
